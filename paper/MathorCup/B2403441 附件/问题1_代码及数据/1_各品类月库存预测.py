...
for category, df in inventory_data.groupby('品类'):
    ...
    if ERROR == 0 and n > 4:
        ...
        result1, data_hat1, _, _, a1, b1 = gm11(train_data, test_num)
        coef_and_action.append([category, '传统GM(1,1)', a1, b1])
        ...
        # plt.savefig(f'prediction_comparison_{category}.png')  # 保存图表
        # plt.close()  # 关闭图表
        # time.sleep(1)  # 等待1秒
        ...
        if choose == 1:
            ...
            coef_and_action.append([category, '传统GM(1,1)_预测', a1, b1])
        elif choose == 2:
            result = new_gm11(data, predict_num)
            data_hat = new_gm11(data, len(data))
            coef_and_action.append([category, '新信息GM(1,1)_预测', None, None])
        elif choose == 3:
            result = metabolism_gm11(data, predict_num)
            data_hat = metabolism_gm11(data, len(data))
            coef_and_action.append([category, '代谢GM(1,1)_预测', None, None])
        ...
        print("Traditional GM(1,1) model prediction details:")
        result1, data_hat1, _, _, a1, b1 = gm11(data, predict_num)
        coef_and_action.append([category, '传统GM(1,1)', a1, b1])
...
# 创建一个包含发展系数和灰度作用量的表格
coef_and_action_df = pd.DataFrame(coef_and_action, columns=['品类', '模型', '发展系数', '灰度作用量'])
coef_and_action_df.to_csv('发展系数和灰度作用量.csv', index=False)

# 输出最终结果
print(inventory_predictions_df)
print(error_results_df)
print(coef_and_action_df)

# 将预测结果、误差结果和发展系数及灰度作用量保存到同一个Excel文件中
wb = Workbook()
ws1 = wb.active
ws1.title = '预测结果'
...
ws3 = wb.create_sheet(title='发展系数和灰度作用量')
for r in dataframe_to_rows(coef_and_action_df, index=False, header=True):
    ws3.append(r)

wb.save('预测与误差与发展系数结果1.xlsx')
