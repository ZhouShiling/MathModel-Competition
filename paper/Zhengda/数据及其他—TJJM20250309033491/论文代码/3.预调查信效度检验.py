        ...
        try:
            ...
            if len(data_clean) > 1:
                ...
                sns.heatmap(corr_matrix,
                            ...
                            cbar_kws={"shrink": 0.8},
                            annot_kws={"size": 10})
                plt.title(f"{section_name}变量间相关系数矩阵", fontsize=16)
                plt.xticks(rotation=45, ha='right', fontsize=10)
                plt.yticks(fontsize=10)
                plt.tight_layout()
                plt.savefig(save_dir / f"{section_name}_变量相关热力图.png", dpi=300, bbox_inches='tight')
                plt.close()
        except Exception as e:
...
def calculate_ave_cr(loadings):
    ...
    ave = loadings_sq.mean(axis=0)
    cr = (loadings.sum(axis=0) ** 2) / (loadings.sum(axis=0) ** 2 + (1 - loadings_sq).sum(axis=0))
    return ave, cr
...
def cfa_analysis(data, model_spec):
    ...
    # 在analyze_scale函数中添加以下内容
    # CFA模型规范（示例）
    cfa_model = '''
    # 定义测量模型
    Human =~ Q9_1 + Q9_2 + Q9_3
    Ethics =~ Q10_1 + Q10_2 + Q10_3
    Tech =~ Q11_1 + Q11_2 + Q11_3
    Learn =~ Q12_1 + Q12_2 + Q12_3
    '''

    # 执行CFA
    cfa_results = cfa_analysis(data, cfa_model)

    # 提取标准化因子载荷
    cfa_loadings = cfa_results[['lhs', 'rhs', 'std_est']].pivot(
        index='rhs', columns='lhs', values='std_est')

    # 计算聚合效度指标
    ave, cr = calculate_ave_cr(cfa_loadings)
    ...
    # 将结果保存到Excel
    with pd.ExcelWriter(f"analysis_results/{section_name}_分析报告.xlsx", mode='a') as writer:
        cfa_loadings.to_excel(writer, sheet_name="CFA因子载荷")
        validity_report.to_excel(writer, sheet_name="聚合效度", index=False)
...
if __name__ == "__main__":
    ...
    try:
        ...
        sections = [
            ('AI能力评估',
             df.loc[:,
             '9、人本思维能力—1.在实践中，我能够主动选择并使用数字工具':'12、12.我能够利用AI技术实现知识的转化'].copy(),
             4),
            ('AI能力影响因素',
             df.loc[:,
             '14、个体认知—1.我能够利用AI技术解决在学习中出现的问题':'17、18.我认为使用AI技术可以更容易达到学习或科研目标'].copy(),
             4),
            ('学习适应量表',
             df.loc[:, '18、学习适应量表—1.我感觉我适应研究生的学习':'19、10.因为学业，放弃课外活动让我感到失望。'].copy(),
             2),
            ('科研效能感量表',
             df.loc[:, '20、科研效能感量表—1.遵循研究伦理':'20、9.发现并报告研究的局限性和未来可能的研究方向'].copy(),
             2)
        ]
        ...
        if not total_scores.empty:
            with pd.ExcelWriter("analysis_results/量表总分相关分析.xlsx") as writer:
                ...
                sns.heatmap(score_corr,
                            ...
                            cbar_kws={"shrink": 0.8},
                            annot_kws={"size": 10})
                plt.title("量表总分间相关系数矩阵", fontsize=16)
                plt.xticks(fontsize=12, rotation=30)
                plt.yticks(fontsize=12)
                plt.tight_layout()
                plt.savefig("visualizations/量表总分相关热力图.png", dpi=300, bbox_inches='tight')
                plt.close()
